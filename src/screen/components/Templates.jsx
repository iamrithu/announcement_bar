import { Card, DataTable, Page, Button, Stack, Layout,TextField } from '@shopify/polaris'
          ...
          <div style={{ display: 'grid', gridTemplateColumns: "auto auto", gap: "10px", padding: "10px 14px" }}>
            {
              templates.map((info, index) => {
                return <div key={index} style={{ height: "40px", width: "100%", background: info.background, fontSize: info.fontSize, color: info.fontColor, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", borderRadius: "4px", }} onClick={() =>choose_template(info)}>{info.name}</div>
              })
            }
          </div>
        </Card>
     ...
     {openTextField?<>
      <Layout.Section oneHalf>
        <Card title="Content Configuration" sectioned>
          <TextField label="Name" value={name} onChange={set_name} autoComplete="off" />
          <TextField label="Content"  value={content} onChange={set_content}autoComplete="off" />
        </Card>
      </Layout.Section>
      <Layout.Section oneHalf>
        <Card title="Design Configuration" sectioned>
          ...
          >
            <lable
              style={{ fontSize: "15px", fontWeight: "200", marginRight: "5%" }}
            >
